...
import static com.bakdata.quick.gateway.directives.topic.rule.validation.ValidationUtility.makeCheckForKeyArgument;
 ...
 *
 * <p>
 * When a user declares a non-mutation or a non-subscription type with a return type that is not a list, they have to
 * provide key information - either keyField or keyArgument. The following example presents the correct way of providing
 * key information (key argument is present when the return type is not a list, and it is the same as the input name
 * (id):
 * <pre>{@code
 * type Product {
 *     id: ID!
 ...
 * }
 * }</pre>
 * On the other hand, the example below depicts a code example in which the rules for providing key information are
 * violated (the directive in the product field does not have the key argument at all, and the one in the url field has
 * an incorrect value of the key argument - it should be productId):
 * <pre>{@code
 * type Query {
